...
@author = 'YuanXu'
@date = '2022/01/27'
功能：三代靶向耐药流程--SGA_001模块
description：PA-4基因衍生，生成点突变和indel结果，并分类
            强突变（strong mutation，SM）：提前终止密码子+Indel
            弱突变（weak mutation，WM）：除了提前终止密码子其它点突变
            野生型（wild， W）：无点突变或者提前终止密码子
            低深度（low depth，LD）：深度<10
"""
...
def run_SGA_001_module(input_fastq_file):
    barcode_name = abstract_barcode_name(input_fastq_file) #得到barcode/sample name
    reference_file = f"../Module/Reference/TMP/{barcode_name}_analysis/SGA_001/SGA_001.fasta"
    ref_fa = ReferenceFasta(reference_file) #ReferenceFasta对象，SNP模块需要ref_fa_names属性
    
    mpileup_file = f"{barcode_name}.SGA_001.mpileup.txt"
    ...
    output_result_file = barcode_name + '_SGA_001_result.txt'
...
if __name__ == '__main__':
    ...
    SGA_001_result = run_SGA_001_module(input_fastq_file)
