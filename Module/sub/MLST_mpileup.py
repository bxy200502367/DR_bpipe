...
@date = '2022/01/27'
功能：三代靶向耐药流程--MLST模块
description：拟合生成序列和MLST数据库匹配，寻找型别
"""
...
def run_MLST_module(input_fastq_file):
    barcode_name = abstract_barcode_name(input_fastq_file) #得到barcode/sample name
    reference_file = f"../Module/Reference/TMP/{barcode_name}_analysis/MLST/MLST.fasta"
    ref_fa = ReferenceFasta(reference_file) #ReferenceFasta对象，SNP模块需要ref_fa_names属性
    
    mpileup_file = f"{barcode_name}.MLST.mpileup.txt"
    ...
    output_result_file = barcode_name + '_MLST_result.txt'
...
if __name__ == '__main__':
    ...
    MLST_result = run_MLST_module(input_fastq_file)
